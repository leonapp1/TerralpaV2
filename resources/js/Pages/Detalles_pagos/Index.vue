                ...
                <div class="flex-auto p-4">
                    ...
                    <div class="min-w-full p-4 bg-white rounded-lg shadow-xs">
                        ...
                        <table class="min-w-full bg-white divide-y divide-gray-200 rounded-lg shadow-md">
                            ...
                            <tbody>
                                <tr v-for="(detalle, index) in paginatedDetallesPagos" :key="detalle.id">
                                    <td class="px-4 py-2">{{ detalle.descripcion }}</td>
                                    <td class="px-4 py-2 flex gap-2">
                                        <button @click="openModalform(2, detalle)"
                                            class="px-4 py-2 text-white bg-yellow-500 rounded hover:bg-yellow-600">Editar</button>

                                        <button @click="delet(detalle.id)"
            ...
            <div class="w-full max-w-3xl p-6 bg-white rounded-lg shadow-lg">
                ...
                <form @submit.prevent="save">
                    <div class="mb-4">
                        ...
                        <input v-model="form.descripcion" id="descripcion" type="text"
...
const props = defineProps({ detalles_pagos: Array });
...
const openModalform = (op, detalle = null) => {
    showmodalform.value = true;
    operation.value = op;
    if (op === 1) {
        title.value = "Registrar Detalle de Pago";
        form.reset();
    } else {
        title.value = "Actualizar Detalle de Pago";
        form.id = detalle.id;
        form.descripcion = detalle.descripcion;
    }
};

const delet = (id) => {
    if (confirm('¿Estás seguro de que deseas eliminar este detalle de pago?')) {
        form.delete(route('detalles_pagos.destroy', id), {
            onSuccess: () => {
                ok("Detalle de Pago Eliminado");
            },
            onError: (errors) => {
                console.error('Error al eliminar el detalle de pago:', errors);
            }
        });
    }
};
...
const save = () => {
    if (operation.value === 1) {
        form.post(route('detalles_pagos.store'), {
            onSuccess: () => {
                ok("Detalle de Pago Creado");
            },
            onError: (errors) => {
                console.error('Error al crear el detalle de pago:', errors);
            }
        });
    } else {
        form.put(route('detalles_pagos.update', form.id), {
            onSuccess: () => {
                ok("Detalle de Pago Actualizado");
            },
            onError: (errors) => {
                console.error('Error al actualizar el detalle de pago:', errors);
            }
        });
    }
};
...
const filteredDetallesPagos = computed(() => {
    return props.detalles_pagos.filter(detalle => {
        return detalle.descripcion.toLowerCase().includes(searchQuery.value.toLowerCase());
    });
});

const paginatedDetallesPagos = computed(() => {
    const startIndex = (currentPage.value - 1) * itemsPerPage;
    return filteredDetallesPagos.value.slice(startIndex, startIndex + itemsPerPage);
});
...
const nextPage = () => {
    if (currentPage.value * itemsPerPage < filteredDetallesPagos.value.length) currentPage.value++;
};

const downloadExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(props.detalles_pagos);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Detalles de Pagos');
    XLSX.writeFile(workbook, 'detalles_pagos.xlsx');
};
</script>
